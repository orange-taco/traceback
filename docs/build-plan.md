...
| Phase 0B — account/auth and common API foundation | PR #5 / `6a0b3d4` | local checks and tests recorded in `docs/current/README.md`; merge commit `489ed3b` | custom User/Auth foundation, account common models, request ID, common error response, pagination, admin auth baseline |
...
- 관리자 REST 인증: JWT Bearer token + `is_staff`/`is_superuser` + DRF `IsAdminUser`
- DB: PostgreSQL

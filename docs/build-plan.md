...
- 정확한 다음 행동: PR #7의 Kakao backend 구현과 검증을 마무리한다. `api-spec.md`의 Kakao start/callback 계약, provider env var, existing email auto-linking 실패 응답 정책과 코드/테스트가 일치하는지 확인한다. Kakao backend 검증 후 `../traceback-client` 작업트리와 client 작업 규칙 파일을 정리하고 frontend account 연동을 시작한다. 이메일 인증 endpoint는 발송 시스템 결정 후 구현한다. Phase 0 완료 전 Phase 1 Catalog를 시작하지 않는다.

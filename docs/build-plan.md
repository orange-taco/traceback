...
- 미해결/설계 의심: PostgreSQL 실환경 migrate/test와 production image build는 이번 세션에서 아직 재검증하지 않음. `User.deleted_at`, `SocialAccount.deleted_at` 컬럼은 탈퇴/익명화 정책을 표현하기 위해 유지하되 실제 탈퇴 처리 메서드/service는 회원 탈퇴 API/운영 플로우가 시작될 때 구현한다. `UserToken`, `EmailChangeRequest`, `BenefitClaim`의 실제 발급/소비 API와 클라이언트 연동 flow는 클라이언트 연동 시점에 구현한다. Kakao social login start/callback, provider env var, 성공/실패 frontend redirect, auto-linking 실패 정책은 다음 백엔드 슬라이스에서 확정한다. Kakao Developers 설정값(`REST API key`, client secret 사용 여부, backend callback URL, frontend 성공/실패 redirect URL)이 구현 전에 필요하다. Google social login은 현재 Phase 0 provider 범위가 아니므로 추가하려면 별도 결정이 필요하다. `../traceback-client`에는 현재 `AGENTS.md`가 없으므로 프론트 작업 전 client-side 작업 규칙 파일을 만들거나 복구한다. `SiteSetting`은 Phase 1 Catalog 또는 Phase 3 Order에서 재검토하고, `IdempotencyRecord`는 Phase 3 Order 전 재결정한다.
- 다음 작업: 이 PR은 email/password JWT backend API와 문서 진입점 정리로 마무리한다. 다음 PR은 Kakao social login backend vertical slice를 먼저 진행하고, 그 다음 client account/social auth 연동으로 넘어간다.
- 정확한 다음 행동: PR #7 문서 정합성과 검증을 마무리한다. merge 후 `development`에서 Kakao social login backend 브랜치를 만든다. `api-spec.md`에 Kakao start/callback path, provider 설정 env var, 성공/실패 redirect, existing email auto-linking 실패 응답 정책을 기록하고 구현한다. Kakao backend 검증 후 `../traceback-client` 작업트리와 client 작업 규칙 파일을 정리하고 frontend account 연동을 시작한다. 이메일 인증 endpoint는 발송 시스템 결정 후 구현한다. Phase 0 완료 전 Phase 1 Catalog를 시작하지 않는다.
